...
\begin{abstract}
BunkerCoin is a blockchain protocol designed for low-bandwidth networks, including shortwave radio links. Its consensus engine, Bunkerglow, adapts Alpenglow~\cite{alpenglow} to the constraints of radio communication.

The proposed network architecture combines reliable P2P links, opportunistic broadcasts, and a location-aware routing service called Sherpa. Sherpa maintains topology and peer information to support block propagation and voting. A central design challenge is aggregating BLS multi-signatures along radio paths to reduce voting traffic. The design targets a 300-byte Maximum Transmission Unit (MTU), with 32:96 erasure coding for broadcast data and reliable ARQ links for P2P transfers.
\end{abstract}

\noindent\textbf{Implementation status.} This working document describes both the current BunkerCoin prototype and proposed protocol extensions. The private testnet implements Bunkerglow consensus and transaction processing over PACTOR radio links. The bridge, verified messaging, and location-attestation mechanisms remain under development.
...
BunkerCoin adopts Alpenglow~\cite{alpenglow}, a consensus protocol originally proposed for the Solana blockchain, to achieve block finality under constrained network conditions. Alpenglow replaces Proof of History and Tower BFT with separate voting and block-dissemination components.
...
\begin{itemize}
    \item \textbf{Network Partitions and Link Failure:} The protocol's P2P-first strategy (see Section~\ref{sec:physical}) provides the baseline transport. If direct links fail, the Sherpa routing service seeks alternative paths through the multi-hop radio mesh. Erasure coding in broadcast modes and ARQ for P2P links protect against transient packet loss. During a network partition, finalization requires sufficient communicating stake to form the required quorums, under the consensus protocol's fault assumptions. Nodes in segments without a quorum pause finalization and repair missing data once connectivity is restored.

    \item \textbf{Validator Downtime:} While Fast-Finalization requires $\geq 80\%$ stake approval, the standard finalization path uses two rounds of $\geq 60\%$ stake approval. Progress depends on Alpenglow's Byzantine-fault and network assumptions~\cite{alpenglow}; quorum thresholds alone do not specify the protocol's fault tolerance.
    ...
    \item \textbf{Bridge Pauses:} In the proposed bridge, a Bridge Certificate (BCert) requires signatures representing at least two-thirds of the total staked weight authorized by the latest Validator Set Update Certificate (VSCert). If insufficient stake is reachable to meet this threshold, bridge transfers pause. Loss of internet connectivity does not reduce the required signing threshold.
\end{itemize}
...
This design keeps account entries compact—most accounts will hold only 1-5 different tokens. A fixed-width token balance entry requires 12 bytes (4 for the ID and 8 for the amount), excluding container and serialization overhead.
...
One of BunkerCoin's primary value propositions is serving as a resilient custody layer for digital assets during periods of internet instability or outright failure. To fulfill this role, the protocol must support asset transfers between BunkerCoin and high-throughput networks like Solana or similar chains. This section describes a proposed bridge architecture that derives signing authority from BunkerCoin's validator set.
...
\begin{enumerate}
    ...
    \item \textbf{An Immutable On-Chain Program} on the target chain (e.g., Solana) that verifies both certificate types. It only accepts a BCert if its signers represent at least two-thirds of the total staked weight authorized by the most recent VSCert.
...
The bridge's root of trust is established by VSCerts. At the end of each epoch, as part of the main state snapshot process, validators certify a message containing the version, the complete list of public keys, and stake weights for the upcoming epoch through the core radio-based consensus. The certificate is ratified under the consensus rules of the previous validator set; it does not require every validator to be online. A relayer submits this VSCert to the on-chain program, which verifies it against the previous validator set to rotate the authorized set for the new epoch.

\subsubsection{Bridge Certificates (BCerts)}
BCerts authorize individual asset transfers, primarily to facilitate the movement of external assets onto BunkerCoin for secure custody. For example, when a user locks native \texttt{SOL} in the bridge program on Solana, internet-connected validators observe the event and produce a BCert to authorize the minting of wrapped \texttt{SOL} on BunkerCoin. Conversely, when a user burns wrapped \texttt{SOL} on BunkerCoin, the validators generate a BCert to authorize the release of the native \texttt{SOL} on Solana. This signed message contains the certificate version and transfer details (amount, asset, source, destination, nonce). The receiving chain verifies the BCert's signatures and requires its signers to represent at least two-thirds of the total staked weight authorized by the most recent VSCert. If valid, the mint or release is executed. If insufficient stake has internet connectivity to meet this threshold, bridge transfers pause until connectivity is restored.

\subsection{Relationship to Federated Models}
Federated bridge models delegate signing authority to a separate committee. BunkerCoin's proposed bridge instead derives that authority from the validator set ratified by its radio-based consensus. Although only internet-reachable validators participate in BCert signing, each BCert must represent at least two-thirds of the total staked weight authorized by the latest VSCert. Transfers pause if this threshold cannot be met.

The BCert signing threshold is distinct from Alpenglow's Byzantine-fault assumptions~\cite{alpenglow}. It does not establish a universal attack-cost bound or, by itself, prove security equivalent to the base chain. Bridge security also depends on correct certificate verification, validator-set rotation, and source-chain finality.
...
\begin{enumerate}
    \item \textbf{Checkpoint Verification:} A state root finalized on an external chain provides an independently verifiable checkpoint. Using it to reject conflicting BunkerCoin histories requires explicit rules for selecting and enforcing trusted checkpoints; those rules remain to be specified.
...
\begin{itemize}
    ...
    \item \textbf{Long-Range and Re-org Attacks:} Consensus safety depends on Alpenglow's fault assumptions~\cite{alpenglow}. Nodes bootstrapping from snapshots also need an authenticated validator history or a trusted checkpoint. Roots anchored on an external chain (Section~\ref{sec:bridge}) can support checkpoint verification. Protection against long-range rewrites requires explicit rules for selecting and enforcing checkpoints.

    \item \textbf{Bridge Security:} The proposed bridge derives its validator-set authority from radio-based consensus and requires each BCert to represent at least two-thirds of the total staked weight authorized by the latest VSCert. This threshold remains fixed during connectivity loss. Correct certificate verification, validator-set rotation, and source-chain finality are also required for bridge security; these mechanisms remain under development.
\end{itemize}
...
\emph{Solana Alpenglow Consensus: Increased Bandwidth, Reduced Latency}.
White paper, version 1.1, July 22, 2025.
